  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-4 flex flex-wrap gap-3 items-center">
      ...
      <div className="flex items-center gap-2">
        {['новинка','унисекс','лимит'].map(t => (
          <button key={t} onClick={()=>toggle(setTags, t)} className={`badge ${tags.has(t)?'bg-gold/20 border-gold/40':''}`}>{t}</button>
        ))}
      </div>
      <div className="flex items-center gap-2">
        {[['hit','🔥'],['star','⭐️'],['gift','🎁'],['bolt','⚡️']].map(([b,ico]) => (
          <button key={b} onClick={()=>toggle(setBadges, b)} className={`badge ${badges.has(b)?'bg-white/10':''}`}>{ico}</button>
        ))}
      </div>
      <div className="flex items-center gap-2 text-sm">
        <span className="text-fog">До</span>
        <input type="range" min="500" max="6000" value={price} onChange={e=>setPrice(Number(e.target.value))}/>
        <span className="text-gold font-semibold">{new Intl.NumberFormat('ru-RU').format(price)} ₽</span>
      </div>
    </div>
  )
}
